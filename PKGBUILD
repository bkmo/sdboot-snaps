...
sha256sums=('SKIP')
    ...
    package() {
    cd $pkg_name-$pkgver
    install -Dm 0644  "/pacman/hooks/05-path-stop.hook" -t "$pkgdir/usr/share/libalpm/hooks/"
    install -Dm 0644  "/pacman/hooks/zz-path-start.hook" -t "$pkgdir/usr/share/libalpm/hooks/"
    install -Dm 0755  "/pacman/scripts/snap-path-pre" -t "$pkgdir/usr/share/libalpm/scripts/"
    install -Dm 0755  "/pacman/scripts/snap-path-post" -t "$pkgdir/usr/share/libalpm/scripts/"
